 ...
 * @license    http://www.fsf.org/licensing/licenses/agpl-3.0.html AGPL3
 */
class Ajgl_Criteria_Criterion_AnyTest
    extends PHPUnit_Framework_TestCase
{
    /**
     * @var Ajgl_Criteria_Criterion_Any
     */
    protected $_criterion;
    
    public function setUp()
    {
        $this->_criterion = new Ajgl_Criteria_Criterion_Any('foo');
    }
    
    public function testConstructor()
    {
        $this->assertEquals('foo', $this->_criterion->getField());
        $this->assertEquals('*', $this->_criterion->getValue());
        $this->assertEquals(
